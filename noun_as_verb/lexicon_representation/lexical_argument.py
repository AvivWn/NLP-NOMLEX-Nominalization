...
from .utils import check_relations, get_word_in_relation
from noun_as_verb.arguments_extractor.extraction.extracted_argument import ExtractedArgument
from noun_as_verb.arguments_extractor.candidates_finders.candidate import Candidate
...
class LexicalArgument:
	...
	def _check_position(self, position: str, candidate: Candidate, matched_argument: ExtractedArgument):
		"""
		...
		elif ARG_CONSTRAINT_REQUIRED_PREFIX in self.constraints and \
			self._check_prefix(candidate_token, self.prefixes) is None:
			return False
	...
	def try_to_match(self, candidate: Candidate, referenced_token: Token):
		"""
		Checks whether the given candidate matches to to this argument
		:param candidate: a candidate for this argument
		...
		if candidate_token != referenced_token and cand_start <= referenced_i <= cand_end:
			return None
