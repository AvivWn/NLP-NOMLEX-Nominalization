...
from .simplify_lexicon_keys import remove_entries, split_entries
from .simplify_entry import rearrange_entry
from .simplify_subcat import nom_roles_for_pval
from .simplify_representation import argument_constraints, missing_required, args_without_pos
from .utils import get_current_specs, curr_specs, is_known, without_part, get_right_value, unknown_values_dict, known_values_dict
	...
	for word in lexicon.keys():
		...
		for subcat_type, subcat in lexicon[word][ENT_VERB_SUBC].items():
			...
			for complement_type in all_complements:
				...
				positions_per_complement[complement_type] += subcat[complement_type][ARG_POSITIONS] + subcat[complement_type][ARG_PREFIXES]
...
def get_summary():
	...
	for collided_args, words in arguments_collisions.items():
		...
		if set(collided_args).issubset(["SUBJECT", "OBJECT", "IND-OBJ", "PP", "PP1", "PP2"]):
...
def add_default_entry(lexicon):
	...
	for word_entry in lexicon.values():
		for subcat in word_entry[ENT_VERB_SUBC].values():
			...
			for complement_type in all_complements:
				...
				default_arg[ARG_PREFIXES] += complement_info.get(ARG_PREFIXES, [])
				default_arg[ARG_POSITIONS] = list(set(default_arg[ARG_POSITIONS] + complement_info.get(ARG_POSITIONS, [])))
				default_arg[ARG_ROOT_UPOSTAGS] = list(set(default_arg[ARG_ROOT_UPOSTAGS] + complement_info.get(ARG_ROOT_UPOSTAGS, [])))
				default_arg[ARG_ROOT_URELATIONS] = list(set(default_arg[ARG_ROOT_URELATIONS] + complement_info.get(ARG_ROOT_URELATIONS, [])))
				default_arg[ARG_ROOT_PATTERNS] = list(set(default_arg[ARG_ROOT_PATTERNS] + complement_info.get(ARG_ROOT_PATTERNS, [])))

				constraints = difference_list(complement_info.get(ARG_CONSTRAINTS, []), [ARG_CONSTRAINT_PLURAL, ARG_CONSTRAINT_N_N_MOD_NO_OTHER_OBJ, ARG_CONSTRAINT_DET_POSS_NO_OTHER_OBJ])
	...
	for complement_type, complement_info in default_subcat.items():
		...
		complement_info[ARG_PREFIXES] = list(set(complement_info[ARG_PREFIXES]))
