...
class ArgsTransferModel(pl.LightningModule):
	"""
		The model learns to determine the role-type of verb's arguments, without context
		Thus, the model can be used also for nominal arguments, where the nominalization is mapped to a verb
	"""

	pretrained_wrapper: PretrainedWrapper # Wraps a pretrained model

	def __init__(self, hyper_params):
		super(ArgsTransferModel, self).__init__()
		...
		self.tagset = {"SUBJECT":0, "OBJECT":1, "NONE":2} #hyper_params.tagset
		self.labels = list(self.tagset.keys())
		self.n_labels = len(self.tagset)
		...
		self.net = ClassifierNetwork(pre_dim, self.n_labels, self.hparams.dropout)

	def forward(self, x):
		token_ids, token_mask = x
	...
	def encode(self, tokens, verb):
		return self.pretrained_wrapper.encode_pair(tokens, verb)
	...
	def _pandas_to_pytorch(self, dataset: pd.DataFrame, dataset_type, encoded_examples:dict):
		...
		for row_info in tqdm(dataset.itertuples(), f"Processing {dataset_type} dataset", leave=True):
			(i, verb, arg, label) = tuple(row_info)
			...
			if not features:
				tokens = arg.split()
				features = self.encode(tokens, verb)
	...
	def val_dataloader(self):
		return [DataLoader(self.train_dataset, batch_size=256, num_workers=4),
				DataLoader(self.val_dataset, batch_size=256, num_workers=4)]

		#return DataLoader(self.val_dataset, batch_size=256, num_workers=4)
	...
	def _calculate_metrics(self, y, y_hat, step_type):
		report = self._get_classification_report(y, y_hat, self.tagset)
