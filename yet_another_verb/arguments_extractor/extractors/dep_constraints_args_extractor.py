...
from yet_another_verb.nomlex.representation.constraints_map import ConstraintsMap, ORConstraintsMaps
...
class DepConstraintsArgsExtractor(ArgsExtractor, abc.ABC):
	...
	def _get_relatives_matched_args(
		...
		for i, relative in enumerate(relatives):
			...
			relative_matched_args = None if relative is None else self._get_matched_arguments(
				words, predicate, relative, constraints_map)
	...
	def _get_matched_arguments(
		...
		for relatives_permute in relatives_permutations:
			relatives_matched_args_combinations = self._get_relatives_matched_args(
				words=words, predicate=predicate, relatives=relatives_permute,
		...
		start_idx, end_idx = min(arg_indices), max(arg_indices)
		new_arg = ExtractedArgument(
			start_idx=start_idx,
			end_idx=end_idx,
			head_idx=words[start_idx: end_idx + 1].root.i,
	...
	def _extract_by_constraints_maps(self, word_idx: int, words: ParsedText, constraints_maps: ORConstraintsMaps) -> Optional[Extractions]:
		...
		extractions = []
		max_num_of_args = 0
		for i, constraints_map in enumerate(constraints_maps):
			if len(constraints_map.included_args) < max_num_of_args:
				break

			try:
				matched_args_combinations = self._get_matched_arguments(words, word, word, constraints_map)
			...
			for matched_args in matched_args_combinations:
				...
				if any(arg.arg_type is not None for arg in matched_args):
					extraction = Extraction(
						words=words, predicate_idx=word_idx, predicate_lemma=word.lemma, predicate_postag=word.pos,
