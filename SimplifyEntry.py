...
def check_subcat_type(entry, subcat_type):
	...
	if subcat_type.replace("-PART-", "-").replace("-PART", "-INTRANS") not in lexicon_fixes_dict.keys():
...
def rearrange_entry_properties(entry):
	...
	for subcat_type in deepcopy(entry).get(ENT_VERB_SUBC, {}).keys():
		...
		# Make sure that the particle is specify in any subcat that should get a particle
